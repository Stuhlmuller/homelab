...
The parent `platform-storage` Application is automated by default and creates
the child `nfs-subdir-external-provisioner` Application in the `argocd`
namespace. The child Application then installs the upstream Helm chart into the

...
- The broad GitHub App credential is intentionally unavailable. Public repository
  clones and public issue/PR reads need no replacement credential. Authenticated
  GitHub writes, CI dispatches, and PR publication remain unavailable until a
  separately reviewed scoped identity is installed. Prepare local changes and
  report that capability gap; do not seek cached tokens, another App key, or a
  PAT fallback. Never put credentials in URLs, output, or repository files.

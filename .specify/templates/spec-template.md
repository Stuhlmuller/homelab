...
- **IAC-001**: Every persistent infrastructure or cluster change MUST identify
  the repository path that owns the desired state.
...
- **IAC-003**: Kubernetes runtime changes MUST identify the Argo CD, Helm,
  Kustomize, raw manifest, or other repo-owned GitOps path that will deliver
  the change, or explicitly state that no Kubernetes runtime change is affected.
...
- [Assumption about delivery, e.g., "No live cluster mutation is required beyond applying repo-authored desired state"]

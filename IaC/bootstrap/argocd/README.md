...
chart `10.0.0`, renders the chart's five component NetworkPolicies, and keeps
the service internal with `ClusterIP`. The policies remain intent-only under
the current kube-flannel data plane; issue #784 tracks effective enforcement.
After Helm succeeds, a Terragrunt `after_hook` waits for the Argo CD Application
and AppProject CRDs, applies the repo-owned `homelab` and `homelab-workloads`
AppProjects, and then applies the `argocd-self-management` Application manifest.
...
issuer are committed here as non-secret desired state; the connector reads its
...
The issuer SSM path is a compatibility copy for the generated Secret. Keep it
aligned with the committed issuer, not `REPLACE_ME`, so future secret refreshes
do not reintroduce a broken provider discovery value.
...
the Entra application registration must emit group membership claims. Entra may
omit the `email_verified` claim from ID tokens; the connector sets
...
the module defaults it calls. Keep OIDC client secrets in AWS Parameter Store,
not in this repository or Helm values.

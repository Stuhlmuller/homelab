...
`TS_AUTH_KEY` repo secret for Tailscale access. The workflows can still fall
back to the legacy `TAILSCALE_AUTH_KEY_SSM_PARAMETER` repo variable when an auth
key needs to be sourced from AWS SSM. Same-repo pull

...
class LiveOpsTests(unittest.TestCase):
    ...
    def test_terragrunt_units_define_live_dependencies(self) -> None:
        ...
        hivemind = (ROOT / "terraform/live/homelab/jobs/hivemind/terragrunt.hcl").read_text()
        ...
        self.assertIn("../../volumes/shared-data", hivemind)
    ...
    def test_live_workload_validation_checks_tailscale_backend_state(self) -> None:
        ...
        self.assertIn("fleetdm_public_url", content)
        self.assertIn("parse_url_host_port()", content)
        self.assertIn("fleetdm_port", content)
        self.assertIn("policy_bot_port", content)
        self.assertIn("urlsplit", content)
        ...
        self.assertIn("hivemind.stinkyboi.com", content)
        self.assertIn("validated Hivemind HTTPS health endpoint", content)
        self.assertIn("policy_bot_auth_status", content)
        self.assertIn("policy_bot_hook_status", content)
        self.assertIn("policy_bot_root_status", content)
        self.assertIn("/api/github/auth", content)
        self.assertIn("/api/github/hook", content)
        self.assertIn("policy-bot root path is unexpectedly public", content)

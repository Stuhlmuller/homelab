...
class SecretManagementTests(unittest.TestCase):
    ...
    def test_github_workflows_prefer_tailscale_oauth_and_support_authkey_fallback(self) -> None:
        ...
        self.assertIn("Fail fast when Tailscale is lock-blocked", action)
        self.assertIn("tailscale status --json", action)
        self.assertIn("health_raw = status.get(\"Health\")", action)
        self.assertIn("isinstance(health_raw, str)", action)
        self.assertIn("status.get(\"BackendState\")", action)
        self.assertIn("\"need login\"", action)
        self.assertIn("pre-authorize OAuth-created tag:ci nodes", action)
        self.assertIn("connectivity-probe-address: 10.1.0.199", plan)
        self.assertIn('connectivity-probe-port: "22"', plan)
        self.assertIn("-refresh=false", plan)
        self.assertIn("connectivity-probe-address: 10.1.0.199", deploy)

    def test_bootstrap_runbook_documents_ci_tailnet_lockout_remediation(self) -> None:
        runbook = (ROOT / "docs/runbooks/bootstrap.md").read_text()

        self.assertIn("## CI Tailnet Lockout Remediation", runbook)
        self.assertIn(".github/actions/setup-infrastructure", runbook)
        self.assertIn("tailscale status --json", runbook)
        self.assertIn("NeedsLogin", runbook)
        self.assertIn("OAuth-created `tag:ci` nodes are pre-authorized", runbook)

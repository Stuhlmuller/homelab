...
class NomadJobTests(unittest.TestCase):
    ...
    def test_openclaw_job_uses_openclaw_codex_runtime(self) -> None:
        content = (ROOT / "nomad" / "jobs" / "openclaw" / "job.nomad.hcl").read_text()
        self.assertIn('job "openclaw"', content)
        ...
        self.assertIn('default = "nomad-primary"', content)
        self.assertIn('attribute = "${node.unique.name}"', content)
        self.assertIn("value     = var.openclaw_nomad_node_name", content)
        self.assertIn('primary: "codex/gpt-5.4"', content)
        self.assertIn("cfg.agents.defaults.model.primary = ref", content)
        self.assertIn("cfg.agents.defaults.contextTokens = 160000", content)
        self.assertIn("memoryGetMaxChars: 12000", content)
        self.assertIn("toolResultMaxChars: 16000", content)
        self.assertIn('forceFlushTranscriptBytes: "1mb"', content)
        self.assertIn("openai-codex/gpt-5.4", content)
        self.assertIn("delete cfg.agents.defaults.models[ref]", content)
        self.assertIn("cfg.channels.discord.heartbeat.showAlerts = false", content)
        self.assertIn("if [ ! -s /data/openclaw/config/openclaw.json ]", content)
        self.assertNotIn("cmp -s ${NOMAD_SECRETS_DIR}/openclaw.bootstrap.json", content)
        self.assertIn("/data/openclaw/home", content)
        self.assertIn("openclaw models status >/tmp/openclaw-models-status.log 2>&1 || true", content)
        self.assertIn("@openai/codex@0.130.0", content)
        self.assertIn("obsidian-cli@0.5.1", content)
        self.assertIn('paths: ["/app/dist/extensions/discord"]', content)
        self.assertIn('dmPolicy: "pairing"', content)
        self.assertIn("showAlerts: false", content)
        self.assertIn("DISCORD_BOT_TOKEN", content)
        self.assertIn('nomadVar "nomad/jobs/openclaw/discord"', content)
        self.assertIn("bot_token", content)
        self.assertIn('HOME                   = "/data/openclaw/home"', content)
        self.assertIn('OPENCLAW_AGENT_RUNTIME = "codex"', content)
        ...
        self.assertIn("/data/openclaw/codex-cli/node_modules/.bin", content)

...
class NomadJobTests(unittest.TestCase):
    ...
    def test_fleetdm_job_routes_through_traefik_with_local_backing_services(self) -> None:
        content = (ROOT / "nomad" / "jobs" / "fleetdm" / "job.nomad.hcl").read_text()
        self.assertIn("fleetdm/fleet:v4.84.1", content)
        self.assertIn('image = "mysql:8.4.8"', content)
        self.assertIn('image = "redis:6.2.21-alpine"', content)
        self.assertIn('default = "nomad-primary"', content)
        self.assertIn('attribute = "${node.unique.name}"', content)
        self.assertIn("value     = var.fleetdm_nomad_node_name", content)
        self.assertIn("traefik.http.routers.fleetdm.rule", content)
        self.assertIn("Host(`fleet.stinkyboi.com`)", content)
        self.assertIn("traefik.http.routers.fleetdm.entrypoints=websecure", content)
        self.assertIn('mode = "bridge"', content)
        self.assertIn('path     = "/healthz"', content)
        self.assertIn('source    = "fleetdm_data"', content)
        self.assertIn("nomad/jobs/fleetdm/config", content)
        self.assertIn('destination = "secrets/fleet-runtime.env"', content)
        self.assertIn('destination = "secrets/mysql_password"', content)
        self.assertIn('MYSQL_ROOT_PASSWORD_FILE = "${NOMAD_SECRETS_DIR}/mysql_root_password"', content)
        self.assertIn('FLEET_MYSQL_PASSWORD_PATH', content)
        self.assertIn('"${NOMAD_SECRETS_DIR}/mysql_password"', content)
        self.assertIn('FLEET_SERVER_ADDRESS', content)
        self.assertIn('"0.0.0.0:1337"', content)
        self.assertIn('FLEET_SERVER_PRIVATE_KEY={{ .server_private_key.Value | trimSpace | toJSON }}', content)
        self.assertIn('FLEET_SERVER_TLS', content)

...
class WorkflowTests(unittest.TestCase):
    ...
    def test_deploy_workflow_uses_safe_wrapper(self) -> None:
        ...
        self.assertNotIn("TS_OAUTH_CLIENT_ID", content)
        self.assertNotIn("TS_OAUTH_SECRET", content)
        ...
        self.assertIn("tailscale-auth-key-parameter: ${{ vars.TAILSCALE_AUTH_KEY_SSM_PARAMETER }}", content)
        self.assertNotIn("oauth-client-id:", content)
        self.assertNotIn("oauth-secret:", content)
        self.assertIn("INGRESS_IP: 10.1.0.200", content)
        self.assertIn("NOMAD_ADDR: http://10.1.0.200:4646", content)
        self.assertIn("NOMAD_HTTP_IP: 10.1.0.200", content)
        self.assertNotIn('USE_TAILSCALE_ENDPOINTS: "1"', content)

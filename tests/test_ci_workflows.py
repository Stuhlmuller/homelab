...
class WorkflowTests(unittest.TestCase):
    ...
    def test_plan_workflow_uses_single_required_job_for_all_prs(self) -> None:
        ...
        self.assertIn("-- -lock=false -input=false -no-color", content)
        self.assertNotIn("-- -refresh=false", content)
    ...
    def test_deploy_workflow_uses_safe_wrapper(self) -> None:
        ...
        self.assertIn("tailscale-auth-key-parameter: ${{ vars.TAILSCALE_AUTH_KEY_SSM_PARAMETER }}", content)
        self.assertIn("INGRESS_IP: ${{ steps.resolve_nomad_endpoint.outputs.nomad_ip }}", content)
        self.assertIn("NOMAD_ADDR: http://${{ steps.resolve_nomad_endpoint.outputs.nomad_ip }}:4646", content)
        self.assertIn("NOMAD_HTTP_IP: ${{ steps.resolve_nomad_endpoint.outputs.nomad_ip }}", content)

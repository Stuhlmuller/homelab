...
| affine | PostgreSQL/pgvector database, Redis append-only cache, uploaded blobs, instance config | `nfs-default` | Coordinated NFS backup plus `pg_dump` before upgrades | Restore PostgreSQL and blob/config claims from one recovery point; restore Redis or accept queued-work loss | Preserve all four claims and keep the ECDSA signing key stable |

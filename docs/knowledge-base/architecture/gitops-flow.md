...
Argo CD Applications are registered through Terragrunt unit templates in
`IaC/.catalog/units/live/argocd-apps/<app>` and generated by the explicit stack
at `IaC/terragrunt.stack.hcl`. The generated units keep the historical
`IaC/live/...` paths so S3 backend keys remain stable. The templates source the
repository-local `IaC/modules/argocd-application-kubernetes` module and pass a

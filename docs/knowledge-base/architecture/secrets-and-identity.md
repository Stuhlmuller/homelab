...
homelab-octelium-public`. The same tunnel is the external callback backbone
  ...
  embedded CA certificate and proves cluster-admin-equivalent resource and
  non-resource access before creating a Secret. Octelium policy, not the
  upstream credential, is the client authorization boundary. Existing Secret
  names are immutable: stage a versioned replacement, switch both catalog
  references in one reviewed change, keep the prior Secret as rollback, and
  validate CI, operator, and Cordium access before closing the rollback window.
  Guarded retirement verifies neither managed Kubernetes Service references the
  old name and confirms deletion. See `docs/octelium.md`.

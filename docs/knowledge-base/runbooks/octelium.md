...
Deluge, Dispatcharr, Grafana, Kiali, LiteLLM, n8n, OctoBot, OpenClaw, Policy
Bot, Prowlarr, Radarr, and Sonarr, plus WEB Service `homelab-demo.homelab`.
The app service primary hostnames are first-level names such as `grafana`,
which publish as existing FQDNs such as `https://grafana.stinkyboi.com`.
...
must only open the Octelium client session and publish Service

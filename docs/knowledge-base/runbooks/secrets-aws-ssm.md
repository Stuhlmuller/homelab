...
| octelium | Octelium native Secret `entra-oidc-client-secret` | Entra OIDC IdentityProvider `entra`; values are generated by `IaC/live/azuread-applications/octelium`, stored in `/homelab/octelium/entra/*`, and copied by `scripts/octelium-entra-oidc.sh` |
...
- Octelium portal SSO is managed through
  `IaC/live/azuread-applications/octelium`; run
  `scripts/octelium-entra-oidc.sh` after the Entra unit applies to refresh the
  Octelium native Secret and IdentityProvider. HUMAN user Entra identifiers are
  runtime-only mappings and must not be committed to git.

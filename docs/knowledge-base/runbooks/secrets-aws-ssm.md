...
| octelium | `entra-client-secret` | Entra OIDC client secret is generated by `IaC/live/azuread-applications/octelium` and copied into the Octelium Cluster with `scripts/octelium-entra-idp.sh` |
...
- Octelium Entra SSO is managed through
  `IaC/live/azuread-applications/octelium` for the Entra app registration and
  `/homelab/octelium/entra/*` SSM values. Run
  `scripts/octelium-entra-idp.sh` after that unit applies so the Octelium
  Cluster receives the `entra-client-secret` Secret and `entra`
  IdentityProvider.

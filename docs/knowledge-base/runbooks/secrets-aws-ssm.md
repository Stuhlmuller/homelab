...
- Deluge VPN profile changes require bumping the non-secret
  ...
  `refreshPolicy: OnChange` and Gluetun reads the Secret from environment at
  pod startup. The profile contract includes private key, peer public key,
  preshared key, interface address, endpoint IP, and endpoint port. Store the
  endpoint as an IP address because Gluetun's custom WireGuard path does not
  accept endpoint DNS names.

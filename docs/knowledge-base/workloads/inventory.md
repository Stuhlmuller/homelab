...
| `dispatcharr` | `media` | `clusters/homelab/apps/dispatcharr` | `IaC/live/argocd-apps/dispatcharr` | persistent all-in-one IPTV stream and EPG manager state on `nfs-default`; Octelium-targeted UI access; provider credentials and playlist URLs stay UI-configured or future ExternalSecret-backed, not committed | cert-manager, istio, platform-storage |

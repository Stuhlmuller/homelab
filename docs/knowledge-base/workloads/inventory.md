...
| `cordium` | `octelium` | `clusters/homelab/apps/cordium` | `IaC/live/argocd-apps/cordium` | Cordium `0.12.7` genesis hook; genesis creates Cordium controller pods and package-managed Octelium `default.cordium` WEB Service; generated Workspace Pods use the dedicated repo-owned `cordium` namespace with narrowly documented privileged Pod Security enforcement; browser access is `https://cordium.stinkyboi.com` with the narrow policy attached to the dedicated `homelab-cordium-user`, workspace app subdomains route through `*.cordium.stinkyboi.com`, and agent automation uses the separate `homelab-cordium-agent` WORKLOAD identity with `cordium-agent-api.homelab` | octelium-cluster, octelium-enterprise |

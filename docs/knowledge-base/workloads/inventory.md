...
| `argocd-image-updater` | `argocd` | `clusters/homelab/apps/argocd-image-updater` | `IaC/live/argocd-apps/argocd-image-updater` | controller state, GitHub App PR credential with approved Contents/Pull requests write access, and Renovate-excluded write-back targets | external-secrets |
...
| `istio` | `istio-system` | `clusters/homelab/apps/istio` | `IaC/live/argocd-apps/istio` | controller state plus the dedicated `octelium-api-ingressgateway` release, SNI-tolerant TLS `Gateway`, and API-only `VirtualService` on NodePort `30443`; the `octelium-api-upnp` CronJob maintains a leased WAN TCP/8443 mapping to that workload for the Cloudflare-proxied Octelium gRPC API | cert-manager |
...
| `cordium` | `octelium` | `clusters/homelab/apps/cordium` | `IaC/live/argocd-apps/cordium` | Cordium `0.12.7` genesis hook plus a native-API PostSync hook for the repo-owned ClusterConfig; genesis creates Cordium controller pods and package-managed Octelium `default.cordium` WEB Service; generated Workspace Pods use the dedicated repo-owned `cordium` namespace with narrowly documented privileged Pod Security enforcement, disposable `cordium-local` storage on `zimaboard-1`, a node-specific Talos user-namespace patch for nested rootless Podman, and a GitOps-managed DaemonSet that converges the same sysctl after sync and node reboot; browser access is `https://cordium.stinkyboi.com` with the narrow policy attached to the dedicated `homelab-cordium-user`, workspace app subdomains route through `*.cordium.stinkyboi.com`, and agent automation uses the separate `homelab-cordium-agent` WORKLOAD identity with `/homelab/cordium/agent-auth-token` and credential-policy access restricted to the package-managed Cordium `ManagementService` | external-secrets, octelium-cluster, octelium-enterprise, platform-storage |
...
| `grafana` | `monitoring` | `clusters/homelab/apps/grafana` | `IaC/live/argocd-apps/grafana` | persistent config, dashboards, platform/workload alert rules with an hourly repeat interval, kubelet-readiness alerting for all four repository-owned PostgreSQL StatefulSets, kube-state-metrics availability, telemetry-gated expected hardware-node and Kubernetes-node alerting, public GitHub API PR/status polling, and stale direct receiver cleanup | external-secrets, cert-manager, istio, prometheus, platform-storage |
...
| `deluge` | `media` | `clusters/homelab/apps/deluge` | `IaC/live/argocd-apps/deluge` | active config on a retained local volume pinned to `zimaboard-0`, with nightly verified NFS archives; shared downloads on QNAP `/media`; SSM-backed WireGuard profile via `deluge-vpn`; guarded recovery for libtorrent session state, the torrent catalog, and already-complete target files; 30-minute startup and runtime liveness windows; `127.0.0.1` Web-to-daemon host compatibility for Sonarr; Prometheus health for the Gluetun VPN and Deluge daemon | cert-manager, istio, platform-storage |
...
| `openclaw` | `ai` | `clusters/homelab/apps/openclaw` | `IaC/live/argocd-apps/openclaw` | persistent runtime state, SSM-backed gateway auth, Discord channel config, SSM-backed GitHub App credentials, Codex OAuth credentials on PVC, explicit agent resource profile, and native HTTP readiness/liveness checks through the proxy to the loopback gateway | external-secrets, cert-manager, istio, litellm, platform-storage |

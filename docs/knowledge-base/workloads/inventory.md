...
| `cordium` | `octelium` | `clusters/homelab/apps/cordium` | `IaC/live/argocd-apps/cordium` | Cordium `0.12.7` genesis hook plus separate user and agent workspace config ConfigMaps; genesis creates Cordium controller pods and Octelium managed services, browser access is `https://cordium.stinkyboi.com` through the Octelium `cordium` WEB Service, and agent automation uses the separate `homelab-cordium-agent` WORKLOAD identity with `cordium-agent-api.homelab` | octelium-cluster, octelium-enterprise |

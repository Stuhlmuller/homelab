...
| `cordium` | `octelium` | `clusters/homelab/apps/cordium` | `IaC/live/argocd-apps/cordium` | Cordium `0.12.7` genesis hook plus a native-API PostSync hook for the repo-owned ClusterConfig; genesis creates Cordium controller pods and package-managed Octelium `default.cordium` WEB Service; generated Workspace Pods use the dedicated repo-owned `cordium` namespace with narrowly documented privileged Pod Security enforcement, disposable `cordium-local` storage on `zimaboard-1`, and a node-specific Talos user-namespace patch for nested rootless Podman; browser access is `https://cordium.stinkyboi.com` with the narrow policy attached to the dedicated `homelab-cordium-user`, workspace app subdomains route through `*.cordium.stinkyboi.com`, and agent automation uses the separate `homelab-cordium-agent` WORKLOAD identity with `/homelab/cordium/agent-auth-token` and credential-policy access restricted to the package-managed Cordium `ManagementService` | external-secrets, octelium-cluster, octelium-enterprise, platform-storage |

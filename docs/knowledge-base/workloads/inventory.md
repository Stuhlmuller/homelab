...
| `grafana` | `monitoring` | `clusters/homelab/apps/grafana` | `IaC/live/argocd-apps/grafana` | persistent config, dashboards, platform/workload alert rules with an hourly repeat interval, kubelet-readiness alerting for all four repository-owned PostgreSQL StatefulSets and Sonarr, kube-state-metrics availability, telemetry-gated expected hardware-node and Kubernetes-node alerting, public GitHub API PR/status polling, and stale direct receiver cleanup | external-secrets, cert-manager, istio, prometheus, platform-storage |
...
| `sonarr` | `media` | `clusters/homelab/apps/sonarr` | `IaC/live/argocd-apps/sonarr` | guarded config recovery to a retained local volume on `zimaboard-0`, with the old NFS claim retained for nightly verified archives and rollback; database state in local `media-postgres`; TV and downloads on QNAP `/media`; Grafana critical alerting for missing or failing kubelet readiness probes; live cutover, alert rollout, and first-backup validation pending | cert-manager, istio, deluge, media-postgres, prowlarr, platform-storage |

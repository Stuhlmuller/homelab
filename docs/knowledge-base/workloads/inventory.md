...
| `istio` | `istio-system` | `clusters/homelab/apps/istio` | `IaC/live/argocd-apps/istio` | controller state plus the dedicated `octelium-api-ingressgateway` release and exact-host TLS `Gateway` on NodePort `30443`; the `octelium-api-upnp` CronJob maintains a leased WAN TCP/443 mapping to that API-only workload for the Cloudflare-proxied Octelium gRPC API | cert-manager |

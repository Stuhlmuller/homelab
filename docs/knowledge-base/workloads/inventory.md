...
| `istio` | `istio-system` | `clusters/homelab/apps/istio` | `IaC/live/argocd-apps/istio` | controller state plus the dedicated TLS-only `octelium-api-nodeport` Service on `30443`; `scripts/octelium-public-dns.sh` maps only WAN TCP/443 to that port for the Cloudflare-proxied Octelium gRPC API | cert-manager |

...
| `istio` | `istio-system` | `clusters/homelab/apps/istio` | `IaC/live/argocd-apps/istio` | controller state plus the dedicated `octelium-api-ingressgateway` release and exact-host TLS `Gateway` on NodePort `30443`; `scripts/octelium-public-dns.sh` maps WAN TCP/443 to that API-only workload for the Cloudflare-proxied Octelium gRPC API | cert-manager |

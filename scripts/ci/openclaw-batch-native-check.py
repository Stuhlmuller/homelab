...
try {
  assert.equal(JSON.parse(fs.readFileSync("/app/package.json", "utf8")).version, "2026.9.5", "unexpected vendor version");
  const files = fs.readdirSync("/app/dist").filter(name => /^io\.audit-.*\.mjs$/.test(name));
  ...
  const baseline = {
    ...
    agents: { defaults: { workspace: "/work/unrelated-workspace" }, entries: { main: {} } },
